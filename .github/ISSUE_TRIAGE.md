...
This document is a hands-on manual for doing issue and pull request triage for `project issues
on GitHub <https://github.com/Falcion/Plusenzustandsmaschine/issues>`_ .
...
- | ``priority/awaiting-more-evidence``: Lowest priority. Possibly useful, but not yet enough
  ...
  | If we think it doesn’t fit in project, we should just say that and why.
...
- ``area/build``: Build and test infrastructure for project
...
- ``area/docker``: Docker use across project's components, such as project Projects and project Models
